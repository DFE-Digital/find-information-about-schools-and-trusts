...
public class AcademiesExportServiceTests
{
    ...
    public async Task ExportAcademiesToSpreadsheet_ShouldCorrectlyExtractAcademyDataAsync()
    {
        ...
        _mockAcademyService.GetAcademiesInTrustDetailsAsync(trustUid).Returns([
            new AcademyDetailsServiceModel("123456", "Academy 1", "Local Authority 1", "Type A", "Urban",
                DateOnly.FromDateTime(DateTime.UtcNow.AddDays(-10)))
        ]);
        _mockAcademyService.GetAcademiesInTrustOfstedAsync(trustReferenceNumber).Returns([
            new SchoolOfstedServiceModel("123456", "Academy 1", now,
                new OfstedShortInspection(now, "School remains Good"),
                new OfstedRating(1, now.AddDays(-1)), new OfstedRating(1, now), false)
        ]);
        _mockAcademyService.GetAcademiesInTrustPupilNumbersAsync(trustReferenceNumber).Returns([
    ...
    public async Task ExportAcademiesToSpreadsheet_ShouldCorrectlyHandleNullValuesAsync()
    {
        _mockAcademyService.GetAcademiesInTrustDetailsAsync(trustUid)
            .Returns([
                new AcademyDetailsServiceModel("123456", null, null, null, null,
                    DateOnly.FromDateTime(DateTime.UtcNow.AddDays(-10)))
            ]);
        ...
        _mockAcademyService.GetAcademiesInTrustPupilNumbersAsync(trustReferenceNumber)
    ...
    public async Task ExportAcademiesToSpreadsheet_ShouldHandleNullPupilData()
    {
        ...
        _mockAcademyService.GetAcademiesInTrustOfstedAsync(trustUid).Returns([
            new SchoolOfstedServiceModel("123456", "Academy 1", now,
                new OfstedShortInspection(now, "School remains Good"), new OfstedRating(-1, null),
