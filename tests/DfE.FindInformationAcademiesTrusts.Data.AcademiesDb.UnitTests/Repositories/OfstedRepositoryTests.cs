...
public class OfstedRepositoryTests
{
    ...
    [Fact]
    public async Task GetSchoolOfstedRatingsAsync_should_combine_establishment_details_with_ofsted_ratings_for_the_urn()
    {
        ...
        using (new AssertionScope())
        {
            ...
            result.IsFurtherEducationalEstablishment.Should().BeFalse();
    ...
    {
        const int urn = 987654;
        SetupEstablishment(urn, "Independent school", dateJoinedTrust);
        _mockAcademiesDbContext.MisMstrEstablishmentFiat.Add(new MisMstrEstablishmentFiat { Urn = urn });

        var result = await _sut.GetSchoolOfstedRatingsAsync(urn);

        result.EstablishmentName.Should().Be("Independent school");
