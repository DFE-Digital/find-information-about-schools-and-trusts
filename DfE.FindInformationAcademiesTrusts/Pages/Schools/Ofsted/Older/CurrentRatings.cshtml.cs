...
public class CurrentRatingsModel(
    ...
    OlderBaseRatingsModel(schoolService, schoolOverviewDetailsService, trustService, dataSourceService,
        ofstedSchoolDataExportService, dateTimeProvider, otherServicesLinkBuilder, schoolNavMenu, ofstedService)
{
    public const string SubPageName = "Older inspections (before November 2025)";

    public override PageMetadata PageMetadata =>
        base.PageMetadata with { SubPageName = SubPageName, TabName = "After September 2024" };

    protected override List<OfstedRating> GetOfstedRating(OlderSchoolOfstedServiceModel ofstedRatings) =>
        ofstedRatings.RatingsWithoutSingleHeadlineGrade;
