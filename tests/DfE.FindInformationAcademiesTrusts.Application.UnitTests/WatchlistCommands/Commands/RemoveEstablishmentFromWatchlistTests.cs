...
public class RemoveEstablishmentFromWatchlistTests
{
    private readonly Mock<IWatchlistRepository> _mockWatchlistRepository;
    private readonly CancellationToken _cancellationToken;

    public RemoveEstablishmentFromWatchlistTests()
    {
        _mockWatchlistRepository = new Mock<IWatchlistRepository>();
        _cancellationToken = CancellationToken.None;
    }

    private RemoveSchoolFromWatchlist.RemoveEstablishmentFromWatchlistCommandHandler CreateHandler() =>
    ...
    public async Task Handle_ValidCommand_RemovesExistingWatchlistRecordAndReturnsTrue()
    {
        ...
        var existing = new Watchlist(new WatchlistId(watchlistId), "100001", null, false, "user@education.gov.uk");
        var command = new RemoveEstablishmentFromWatchlistCommand(watchlistId);
    ...
    {
        var command = new RemoveEstablishmentFromWatchlistCommand(Guid.NewGuid());
    ...
    {
        var watchlistId = Guid.NewGuid();
        var existing = new Watchlist(new WatchlistId(watchlistId), "100001", null, false, "user");
        var command = new RemoveEstablishmentFromWatchlistCommand(watchlistId);
