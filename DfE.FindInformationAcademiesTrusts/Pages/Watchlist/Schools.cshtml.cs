...
public class Schools(IWatchlistQueryService watchlistQueryService, IGetEstablishmentsTemp getEstablishments)
    : ContentPageModel
{
    ...
    public int TrustsCount { get; set; }

    public async Task OnGetAsync(CancellationToken cancellationToken)
    {
        CurrentUser = User.Identity?.Name;

        var establishmentsWatchlist =
            await watchlistQueryService.GetAllEstablishmentsForUser(CurrentUser ?? string.Empty, cancellationToken);
        ...
        var trustsWatchlist =
            await watchlistQueryService.GetAllTrustsForUser(CurrentUser ?? string.Empty, cancellationToken);
